<template>
    <div class="content" >        
        ...
        <coform v-if="showForm" 
            @hidechild="hidechild" 
            v-bind:data="this.coachingOpportunity"
            v-bind:loblist="lobs"
            v-bind:selectedlobids="this.coachingOpportunitySelectedLOBids"></coform>
...
<script>
    ...
    import coform from "@/components/admin/admin_components/coachingopp-form.vue";
    export default {
        name: 'CoachingOpportunity',
        components: { headertitle,coform },
        data() {
            return {
                pagename: 'Coaching Opportunity',
                ...
                showForm: false,
                isReloadGrid: false,
                coachingOpportunitySelectedLOBids: []
            }
        },
        mounted() {
            this.getAllcoachingOpportunities();
        },
        watch: {
            filters: {
                handler() {
        ...
        computed: {
            filteredList() {             
                let items = this.coachingOpportunities;
        ...
        methods: {
            async getAllcoachingOpportunities() {
                if(this.isReloadGrid){
                    let result = await axios.get('/api/CoachingOpportunity/refresh');
                    if(result.data.isAuthenticated == false){
                        ...
                        this.coachingOpportunities = result.data.models.coachingOpportunitiesViewModel;
                    }                    
                }else{
                    let result = await axios.get('/api/CoachingOpportunity/all');
                    if(result.data.isAuthenticated == false){
                         this.$dialog.alert('Session expired!!!',{html:true});
                        this.$store.commit('clearData');
                        axios.post('/api/Authenticate/logout');
                        ...
                        this.coachingOpportunities = result.data.models.coachingOpportunitiesViewModel;
                        this.lobs = result.data.models.lobsViewModel;
                    }                    
                }
            },            
            scrolledToEnd($event) {
            ...
            showDataToEdit(coachingOpportunity) {
                this.showForm=true;
                this.coachingOpportunity=coachingOpportunity;
                this.coachingOpportunitySelectedLOBids = coachingOpportunity.lobViewModels;
            },
            addNew(){
                this.showForm=true;
                this.coachingOpportunity={};
            },
            ...
            async remove(dialog,id){
                if(id>0){
                    await axios.put(`/api/coachingOpportunity/delete/${id}`);
                    this.getAllcoachingOpportunities();
